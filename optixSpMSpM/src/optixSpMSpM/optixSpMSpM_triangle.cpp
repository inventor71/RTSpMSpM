...
typedef SbtRecord<TriangleData>  TriangleDataRec;
...
void mat1ToGPU( const std::string& filePath, optixState& state )
{
    ...
    while (std::getline(file, line)) {
        ...
        if ((val < minval) || (minval == -100000.f)) minval = val;
        if ((val > maxval) || (maxval == 100000.f)) maxval = val;
...
/**
 * Load in Triangle from matrix 2 data file
 */
void storeTriangleData( optixState& state, std::string matrixFile2 )
{
    //
    // Matrix File Input
    //
    std::string triangleFileName = sutil::sampleDataFilePath( matrixFile2.c_str() );
    Triangle        triangle(state.context, triangleFileName, state.minval, state.maxval);

    float *output, *d_output;
    std::vector<float> triangleColIndicies = triangle.value(); // std::vector<float>
    uint64_t cnt = triangle.points().size();

    // Allocate space for matrix
    output = triangleColIndicies.data();
    CUDA_CHECK(cudaMalloc(&d_output, cnt*sizeof(float)));
    cudaMemcpy(d_output, output, cnt*sizeof(float), cudaMemcpyHostToDevice);
    state.triangle_size = cnt / 3; // # of triangles
    state.triangleColIndicies = d_output;

    state.devicePoints = 0;

    state.m_result_dim = triangle.printDim();  //Fill in result buffer (state.m_result_dim.first, dummy value)

    createOnDevice( triangle.points(), &state.devicePoints ); // triangle vertices
}
...
void buildGAS(optixState& state)
{
    ...
    uint64_t numVert = state.triangle_size;

    triangle_input.type                      = OPTIX_BUILD_INPUT_TYPE_TRIANGLES;
    triangle_input.triangleArray.numVertices = numVert * 3;
    triangle_input.triangleArray.vertexFormat  = OPTIX_VERTEX_FORMAT_FLOAT3;
    triangle_input.triangleArray.vertexBuffers = &state.devicePoints;
...
void createModule (optixState& state)
{
    ...
    OPTIX_CHECK_LOG( optixBuiltinISModuleGet( state.context, &module_compile_options, &state.pipeline_compile_options,
                                                &builtin_is_options, &state.triangle_module ) );
}
...
void createProgramGroups (optixState& state)
{
    ...
    hitgroup_prog_group_desc.hitgroup.moduleIS            = state.triangle_module;
...
void createSbt ( optixState& state )
{
    ...
    size_t      hitgroup_record_size = sizeof( TriangleDataRec );
    CUDA_CHECK( cudaMalloc( reinterpret_cast<void**>( &hitgroup_record ), hitgroup_record_size ) );
    TriangleDataRec hg_sbt;
    hg_sbt.data.triangleColIndicies = state.triangleColIndicies;
    ...
    hg_sbt.data.matrix2size = state.triangle_size;
    ...
    state.sbt.hitgroupRecordStrideInBytes = sizeof( TriangleDataRec );
...
int main( int argc, char* argv[] )
{
    ...
    try
    {
        ...
        nvtxRangePop();
        nvtxRangePushA("storeTriangleData");
        storeTriangleData(state, matrix2File);    // Mat2
        nvtxRangePop();

        Timing::startTiming("Optix Preprocessing");
        buildGAS ( state );
        nvtxRangePop();
        ...
        createSbt ( state );
        double preprocess_time = Timing::stopTiming(true);
        ...
        double launch_time = Timing::stopTiming(true);
        std::cout << "LAUNCH_TIME = " << launch_time << " ms" << std::endl;
        nvtxRangePop();
        ...
        //
        {
            ...
            CUDA_CHECK( cudaFree( reinterpret_cast<void*>( state.triangleColIndicies    ) ) );
            ...
            OPTIX_CHECK( optixModuleDestroy( state.triangle_module ) );
